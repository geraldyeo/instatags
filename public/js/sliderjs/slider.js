(function () {
    ...
    tmplSlider = function (o) {
        ...
        slider.find('.slide-images').append($.map(o.slides, function (slide) {
            ...
            return $('<div class="slide-image" data-username="' + slide.username + '" data-fullname="' + slide.fullname + '" data-profilepicture="' + slide.profilepicture + '" data-likecount="' + slide.likecount + '">' + (slide.link ? '<a href="' + slide.link + '" target="_blank">' : '') + '<img src="' + slide.src + '">' + (slide.name ? '<span class="caption">' + slide.name + '</span>' : '') + (slide.link ? '</a>' : '') + '</div>')[0];
        }));
    ...
    Slider = (function () {
        ...
        Slider.prototype.slide = function (num) {
            if (this.slides && this.pages) {
                ...
                this.node.trigger('changed', [this.slides.eq(num), num]);
            }
            this.current = num;
            return this;
        };
